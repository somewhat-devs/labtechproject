...
<body>
	...
	<%
		try {
		String connectionURL = "jdbc:mysql://localhost:3307/labtech";
	...
	<div class="row">
		<div class="col-lg-12">
			...
			<span id="submit-errormsg" class="errormsg">*Please fill
				atleast one of the inputs.</span> <img id="loadimg" src="images/load.gif">
			<br>	
			<div>  </div>										<!-- dhruv -->
			<br>
			<br>
	...
	<div class="row">
		<div class="col-lg-12">
			...
			<br>
			<button id="removeallbtn" class="btn btn-primary"> Remove All</button>									<!-- muskan -->
			<br>
...
$(document).ready(function() {
	...
	$(document).on("click", '.add-btn', function() { 				// block for managing action of qty button
		...
		"<td>  <button class=\"btn btn-outline-success btn-sm rem-btn\">remove item</button>  </td></tr>"	;													// remove button   - niharika
		$('#bill-tbl tbody').append(item_tr);
	});
	$(document).on("click", '.rem-btn', function() { 
		$(this).parent().parent().remove();// block for managing action of add button
	});
	function createTbl() {
		$('#main-tbl').DataTable({
			"pageLength": 5,
